...
public class Nextday {
	...
	public static time nextday(time t) {
		...
		time t1 = null;
		if(t.day<32&&t.day>0&&t.month>0&&t.month<13&&t.year>1811&&t.year<2012) {
			for(int i=0;i<arr_month.length-1;i++) {
				if(t.month==arr_month[i]) {
					if(i<11){
						...
						if((t.day==30&&i>7&&i<10)||(t.day==31&&i<8)) {
							t1.day=1;
							if(t.month!=12) {
								t1.month=t.month+1;
								t1.year=t.year;
		...
		System.out.println("The next day of this day is:"+t1.day+"/"+t1.month+"/"+t1.year);
		return t1;
	}
